...
class MovimentacaoRequests {
    ...
    private async fetchComFallback(
        method: string,
        body?: any,
        id?: number
        ...
        if (ultimoErro) {
            throw new Error(
                "Rota de movimentação não encontrada no backend. Verifique se a API expõe algum destes endpoints: /api/movimentacoes, /api/movimentacao, /movimentacoes ou /movimentacao."
            );
        }
